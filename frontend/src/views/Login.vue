...
<template>
  <div v-show="step == 'username'">
    ...
    <div class="flex one">
      <div v-if="occupancy.currently < (occupancy.historical - 2)" style="text-align: center;">💚 less busy than usual (<span :title="occupancy.currently + ' checkins last 2 hours where the average is ' + occupancy.historical + ' for this hour/day'">curr {{ occupancy.currently }} avg {{ occupancy.historical }}</span>)</div>
      <div v-if="(occupancy.currently >= (occupancy.historical -2)) & (occupancy.currently <= (occupancy.historical + 2))" style="text-align: center;">💛 average busyness (<span :title="occupancy.currently + ' checkins last 2 hours where the average is ' + occupancy.historical + ' for this hour/day'">curr {{ occupancy.currently }} avg {{ occupancy.historical }}</span>)</div>
      <div v-if="occupancy.currently > (occupancy.historical + 2)" style="text-align: center;">🔥 more busy than usual (<span :title="occupancy.currently + ' checkins last 2 hours where the average is ' + occupancy.historical + ' for this hour/day'">curr {{ occupancy.currently }} avg {{ occupancy.historical }}</span>)</div>
    </div>
  </div>
